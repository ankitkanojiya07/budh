...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ref={ref}
      className={`fixed inset-x-0 top-0 z-40 w-full ${className}`}
    >
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    <motion.div
      ...
      className={`fixed inset-x-0 top-0 z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between bg-transparent px-0 py-2 lg:hidden ${visible ? "bg-white/80 dark:bg-neutral-950/80" : ""} ${className}`}
    >
      {children}
    </motion.div>
  );
  ...
  const variantStyles = {
    primary:
      visible 
        ...
        : "bg-transparent text-[#ca3600] shadow-none",
